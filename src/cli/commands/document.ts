...
const generateProperty = (
  ...
  return `${prefix}<div class="${fieldName}">
${prefix}  <label for="${fieldName}">${label}</label>
${prefix}  <span name="${fieldName}">${value}</span>
${prefix}</div>`;
};
...
const generateRates = (planName: string, plan: PlanObject): string => {
  ...
  const content = !rates
    ...
    : Object.keys(rates).map((path) => generateRate(path, rates[path]));
...
const generateQuotas = (planName: string, plan: PlanObject): string => {
  ...
  const content = !quotas
    ...
    : Object.keys(quotas).map((key) => generateQuota(key, quotas[key]));
...
const generateRate = (path: string, rate: PathObject): string => {
  return generateRateOrQuota('rate', path, rate);
};
const generateQuota = (path: string, quota: PathObject): string => {
  return generateRateOrQuota('quota', path, quota);
};

const generateRateOrQuota = (type: string, path: string, po: PathObject): string => {
  const limits = generateLimitsForPath(path, po);
